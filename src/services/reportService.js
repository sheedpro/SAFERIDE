...
async function assignOperationalRecipient(report, target, reason) {
  const station = target.station || (target.type === 'station' ? target.target : null);
  const officer = target.officer || null;
  const recipient = officer || station;
  const assignment = {
    case_id: report.case_id,
    checkpoint_id: target.target?.checkpoint_id || null,
    station_id: station?.station_id || null,
    officer_badge_id: officer?.badgeId || null,
    recipient_type: officer ? 'officer' : 'station',
    recipient_name: officer?.name || station?.name || 'Unassigned operational recipient',
    assignment_reason: reason,
  };
  let delivery = { delivery_status: 'not_configured' };
  if (recipient?.whatsapp) {
    try {
      const message = await twilio.sendOfficerAlert(recipient.whatsapp, report);
      delivery = { delivery_status: 'sent', delivery_message_sid: message.sid };
    } catch (error) {
      console.error(`Operational alert delivery failed for ${report.case_id}:`, error.message);
      delivery = { delivery_status: 'failed', delivery_error: error.message };
    }
  }
  const { error } = await supabase.from('report_assignments').insert({ ...assignment, ...delivery });
  if (error) throw error;
...
async function dispatch(phone, hash, draft, status = 'Dispatched') {
  ...
  report.assigned_station_id = target.station?.station_id || (target.type === 'station' ? target.target?.station_id : null); report.assigned_officer_badge_id = target.officer?.badgeId || null;
  ...
  await assignOperationalRecipient(report, target, target.officer ? 'Primary on-duty officer assigned at the selected checkpoint' : 'No eligible checkpoint officer; station fallback assigned');
...
async function authoriseOfficer(report, phone) {
  ...
  if (error) throw error;
  return (checkpoint?.duty_officers || []).find(officer => officer.onDuty && (!report.assigned_officer_badge_id || officer.badgeId === report.assigned_officer_badge_id) && normalisePhone(officer.whatsapp || '') === normalisePhone(phone)) || null;
}
...
async function rerouteAfterNotSeen(report, badgeId) {
  ...
  const changes = { status: 'Dispatched', officer_action: 'NOTSEEN', officer_action_at: new Date().toISOString(), officer_badge_id: badgeId || null, assigned_station_id: target.station?.station_id || null, assigned_officer_badge_id: target.officer?.badgeId || null, interception_attempts: attempts, redispatch_count: (report.redispatch_count || 0) + 1, dispatched_checkpoint_id: target.target.checkpoint_id, dispatch_target_name: target.target.name, distance_ahead_km: target.distanceAheadKm, estimated_eta_minutes: target.etaMinutes, intercept_lat: target.targetLocation.lat, intercept_lng: target.targetLocation.lng, predicted_lat: target.predictedLocation.lat, predicted_lng: target.predictedLocation.lng, prediction_confidence: target.predictionConfidence };
  const { data, error } = await supabase.from('reports').update(changes).eq('case_id', report.case_id).select().single(); if (error) throw error;
  await assignOperationalRecipient(data, target, target.officer ? 'Next checkpoint officer assigned after a not-seen outcome' : 'No eligible next-checkpoint officer; station fallback assigned');
  return { report: data, rerouted: true };
}
module.exports = { dispatch, findByCase, updateOutcome, authoriseOfficer, notifyReporterOutcome, notifyEscalation, assignOperationalRecipient };
