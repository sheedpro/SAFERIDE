...
router.patch('/cases/:caseId/reassign', requirePermission('police:write'), async (req, res, next) => { try { const { checkpoint_id, reason } = req.body; if (!checkpoint_id || !reason?.trim()) return res.status(422).json({ error: 'Destination checkpoint and reason are required' }); const [{ data: before, error: reportError }, { data: checkpoint, error: checkpointError }] = await Promise.all([supabase.from('reports').select('*').eq('case_id', req.params.caseId).single(), supabase.from('checkpoints').select('*').eq('checkpoint_id', checkpoint_id).single()]); if (reportError) throw reportError; if (checkpointError) throw checkpointError; const { data, error } = await supabase.from('reports').update({ dispatched_checkpoint_id: checkpoint.checkpoint_id, dispatch_target_name: checkpoint.name, status: 'Dispatched', officer_badge_id: null, officer_action: 'REASSIGNED', officer_action_at: new Date().toISOString() }).eq('case_id', req.params.caseId).select().single(); if (error) throw error; const twilio = require('../services/twilioClient'); await Promise.allSettled((checkpoint.duty_officers || []).filter(item => item.onDuty).map(item => twilio.sendOfficerAlert(item.whatsapp, data))); await record(req.admin, 'police-reassign-case', 'report', data.case_id, { before: cleanCase(before), after: cleanCase(data), reason }); res.json(cleanCase(data)); } catch (error) { next(error); } });
