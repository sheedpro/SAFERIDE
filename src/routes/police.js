...
router.post('/officers', requirePermission('police:write'), async (req, res, next) => { try { const { checkpoint_id, name, badgeId, whatsapp, routeIds = [] } = req.body; if (!checkpoint_id || !name || !badgeId || !whatsapp) return res.status(422).json({ error: 'Checkpoint, name, badge ID, and WhatsApp number are required' }); const { data: checkpoint, error } = await supabase.from('checkpoints').select('*').eq('checkpoint_id', checkpoint_id).single(); if (error) throw error; const officers = [...(checkpoint.duty_officers || []).filter(item => item.badgeId !== badgeId && item.whatsapp !== whatsapp), { name, badgeId, whatsapp, routeIds, onDuty: true }]; const { error: updateError } = await supabase.from('checkpoints').update({ duty_officers: officers }).eq('checkpoint_id', checkpoint_id); if (updateError) throw updateError; await record(req.admin, 'police-create-officer', 'officer', badgeId, { after: { name, badgeId, checkpoint_id, routeIds } }); res.status(201).json({ name, badgeId, checkpoint_id, onDuty: true }); } catch (error) { next(error); } });
