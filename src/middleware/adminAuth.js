...
async function authenticateAdmin(req, res, next) {
  try {
    ...
    const { data: admin, error } = await supabase.from('admin_users').select('*').eq('auth_user_id', auth.user.id).eq('is_active', true).maybeSingle();
    if (error) throw error;
    if (!admin) return res.status(403).json({ error: 'This account has no SafeRide admin access' });
    req.admin = admin; next();
  } catch (error) { next(error); }
}
function requirePermission(permission) { return (req, res, next) => can(req.admin.role, permission) ? next() : res.status(403).json({ error: 'Insufficient permission' }); }
